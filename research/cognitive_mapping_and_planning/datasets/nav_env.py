...
class MeshMapper(Building):
        ...
        if self.task_params.move_type == 'circle':
            ...
            for instance_ in instances_:
                ...
                while len(instance) <= self.task_params.num_steps:
                    while self.take_action([instance[-1]], [3])[0] == instance[-1] and len(
                            instance) <= self.task_params.num_steps:
    ...
    def worker(self, instances, perturbs):
        ...
        if self.task_params.output_incremental_transform:
            ...
            incremental_locs[:, :, 0] = np.sum(incremental_locs_ * np.concatenate((np.cos(t), np.sin(t)), axis=-1),
                                               axis=-1)
            incremental_locs[:, :, 1] = np.sum(
                incremental_locs_ * np.concatenate((np.cos(t + np.pi / 2), np.sin(t + np.pi / 2)), axis=-1), axis=-1)
